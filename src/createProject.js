...
function removeTodoInProject (e) {
  if (e.target.className === 'delete-todo-project') {
    ...
    const projectOfTodoId = e.target.parentNode.parentNode.getAttribute('data-list-id');
 
    let projectOfTodo = projectslist.filter(project => `${project._id}--` === projectOfTodoId);
    const todoToRemoveFromProject = projectOfTodo[0]._list.filter(todo => todo._id === id);
    todoToRemoveFromProject[0]._projectId = "";
    
    saveTodo();

    projectOfTodo[0]._list = projectOfTodo[0]._list.filter(todo => todo._id !== id);
